...
function CardDetailModal({
  ...
  const [activeTab, setActiveTab] = useState<'dados' | 'utm' | 'atividades' | 'historico' | 'anotacoes'>('dados')
  const [activityFilter, setActivityFilter] = useState<'todas' | 'pendentes' | 'concluidas'>('todas')

  const isEmerald = card.category === 'LEAD AP'

  const initials = card.linkedContact?.name.split(' ').map(n => n[0]).slice(0, 2).join('').toUpperCase() || '?'
